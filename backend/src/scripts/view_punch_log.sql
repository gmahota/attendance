...
, tab1.shiftDescription, tab1.json, time(date) /60 tempoEmMinutos, case when row_num %2 !=0 then 'Entrada' else 'Saida' end AS punchType from (
...
select * from view_PunchDailyDetails
alter view view_PunchDailyDetails
as
select v.data, v.userid, v.userName,userGroup,v.shiftid, v.description, max(v.entrada) entrada, timeIn entradashift,max(v.saida) saida, timeOut saidashift, shiftSupposedGracePerior from 
	(
