...
function Excell() {
  function fillData(items) {
    XlsxPopulate.fromBlankAsync().then((workbook) => {
      ...
      workbook.sheet("Sheet1").cell(`A2`).value("Date").style({leftBorder: true, topBorder: true});
      ...
      workbook.sheet("Sheet1").cell(`E1`).value("First Time").style("border", true);
      workbook.sheet("Sheet1").cell(`E2`).value("Hora de Chegada Planeada").style("border", true);
      workbook.sheet("Sheet1").cell(`F2`).value("Hora de Chegada").style("border", true);
      workbook.sheet("Sheet1").cell(`G2`).value("Tempo de Atraso").style("border", true);

      workbook.sheet("Sheet1").cell(`H2`).value("Hora Planeada de Saida 1").style("border", true);
      workbook.sheet("Sheet1").cell(`I2`).value("Hora de Saida").style("border", true);
      ...
      items.forEach((item) => {
        workbook.sheet("Sheet1").cell(`A${row}`).value(item.Date).style({leftBorder: true, topBorder: true, rightBorder: true, bottomBorder: true});
        workbook.sheet("Sheet1").cell(`B${row}`).value(item.Name).style("border", true);
        workbook.sheet("Sheet1").cell(`C${row}`).value(item.UserId).style({leftBorder: true, topBorder: true, rightBorder: true, bottomBorder: true});
        workbook.sheet("Sheet1").cell(`D${row}`).value(item.Shift).style({leftBorder: true, topBorder: true, rightBorder: true, bottomBorder: true});;
        workbook
          .sheet("Sheet1")
          .cell(`E${row}`)
          .value(item.firstTime?.clockInDefault).style("border", true);
        workbook.sheet("Sheet1").cell(`F${row}`).value(item.firstTime?.clockIn).style("border", true);
        workbook
          .sheet("Sheet1")
          .cell(`F${row}`)
          .value(item.firstTime?.clockOut).style("border", true);
        workbook
          .sheet("Sheet1")
          .cell(`G${row}`)
          .value(item.firstTime?.clockOutDefault).style("border", true);

        workbook
          .sheet("Sheet1")
          .cell(`H${row}`)
          .value(item.secondTime?.clockInDefault).style("border", true);
        workbook
          .sheet("Sheet1")
          .cell(`I${row}`)
